$(document).ready(function() {
	...
	$("#action_create_invoice").click(function(e) {
		...
		// Get the input date and the current date
		const date1 = convertToDateFormat(date1Input);
		const date2 = convertToDateFormat(date2Input);
	
		if (date1.getTime() >= date2.getTime()) {
			console.log('pass');
			var $btn = $("#action_create_invoice").button("loading");

			$("#response .message").html("<strong>Error</strong>: Wrong due date.");
	...
	function deleteCustomer(userId) {
   	...
   	function updateInvoice() {
		...
		console.log($("#update_invoice").serialize());
		...
		jQuery.ajax({
			...
			error: function(xhr) {
				let errorMessage;
				try {
					let errorData = JSON.parse(xhr.responseText); // Attempt to parse the response
					errorMessage = "<strong>" + errorData.status + "</strong>: " + errorData.message;
				} catch (e) {
					// Handle JSON parsing errors
					errorMessage = "<strong>Error</strong>: Unable to process request. Details: " + e.message;
				}
